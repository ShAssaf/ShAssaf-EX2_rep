...
public class Day {
	...
	//--------------Removing Methods--------------
		...
		public Map<String, ArrayList> create_temporal_array(){
			...
			ArrayList<Pair<Integer, LocalDateTime>> ls =
					new ArrayList<Pair<Integer, LocalDateTime>>();
			int idx = 1;
			for(Event event : this.get_events_arr()) {
				ls.add(new Pair<Integer, LocalDateTime>(idx, event.get_date()));
				ls.add(new Pair<Integer, LocalDateTime>(idx,
						event.get_date().plusMinutes(event.get_duration())));
				idx++;
			}
